...
async function getBalances(address) {
  const balanceBigInt = await hre.ethers.provider.getBalance(address);
  return hre.ethers.formatEther(balanceBigInt); // changed utils.formatEther -> formatEther
}
...
async function main() {
  ...
  console.log("Address of contract:", await contract.getAddress()); // FIXED
  ...
  const amount = { value: hre.ethers.parseEther("1") }; // FIXED: utils.parseEther -> parseEther
  await contract.connect(from1).buyChai("from1", "Very nice chai", amount);
  await contract.connect(from2).buyChai("from2", "Very nice course", amount);
